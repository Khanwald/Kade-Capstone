...
open class CrosswordsGenerator {
    ...
    open func toArray() -> [[String]]{
        var grid:[[String]] = [[String]](repeating: [String](repeating: "-", count: rows), count: columns)
        
        for x in 1...rows{
            for b in 1...columns{
                grid[x - 1][b - 1] = getCell(b, row: x)
            }
        }
    ...
    open func bestResult(terms:Array<CrosswordsGenerator.Word>){
        self.grid = Array2D(columns: columns, rows: rows, defaultValue: emptySymbol)
        currentWords.removeAll()
        resultData.removeAll()
        ...
        for term in terms {
            ...
            let dir = direction == .horizontal ? 0 : 1

    
            setWord(term.column, row: term.row, direction: dir, word: term.word, force: true)
        }
    }
...
//crosswordsGenerator.words = ["apple", "banana", "cherry", "date", "elderberry", "fig", "grape", "huckleberry", "kiwi", "lemon"]
...
//}
//
//var grid:[[String]] = [[String]](repeating: [String](repeating: "-", count: crosswordsGenerator.rows), count: crosswordsGenerator.columns)
//
//for x in 1...crosswordsGenerator.rows{
//    for b in 1...crosswordsGenerator.columns{
//        grid[x - 1][b - 1] = crosswordsGenerator.getCell(b, row: x)
//    }
//}
